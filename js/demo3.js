 ...
 */
{
	...
	const TiltObj = function(el, options) {
		...
		this.DOM.author = this.el.querySelector('.content__desc');
		this._initEvents();
	}
	...
	TiltObj.prototype._initEvents = function() {
		...
		this.mouseleaveFn = (ev) => {
			requestAnimationFrame(() => {
				anime({
					targets: [this.DOM.img, this.DOM.title, this.DOM.author],
					duration: 1500,
	...
	TiltObj.prototype._layout = function(ev) {
		...
		const t = {
			...
			author: this.options.movement.author.translation,
		};
		...
		const transforms = {
			...
			author : {
				x: (-1*t.author.x - t.author.x)/bounds.width*relmousepos.x + t.author.x,
				y: (-1*t.author.y - t.author.y)/bounds.height*relmousepos.y + t.author.y
			}
		};
		...
		this.DOM.author.style.WebkitTransform = this.DOM.author.style.transform = 'translateX(' + transforms.author.x + 'px) translateY(' + transforms.author.y + 'px)';
	};
